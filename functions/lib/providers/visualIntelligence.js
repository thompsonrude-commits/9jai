...
exports.MODE_MODELS = exports.NEGATIVE_PROMPTS = void 0;
exports.detectVisualMode = detectVisualMode;
exports.extractBrandName = extractBrandName;
exports.buildEnhancedPrompt = buildEnhancedPrompt;
exports.buildVisualRequest = buildVisualRequest;
exports.scoreImageQuality = scoreImageQuality;
// ── Cryptographic seed — truly random every call ───────────────────────────
function generateEntropicSeed() {
...
// ── Mode detector ──────────────────────────────────────────────────────────
function detectVisualMode(prompt) {
    const lower = prompt.toLowerCase();
    if (lower.includes('logo') || lower.includes('brand identity') || lower.includes('company logo') || lower.includes('business logo'))
        return 'logo';
    if (lower.includes('icon') || lower.includes('app icon') || lower.includes('favicon'))
        return 'icon';
    if (lower.includes('poster') || lower.includes('flyer') || lower.includes('banner') || lower.includes('thumbnail') || lower.includes('cover art'))
        return 'poster';
    if (lower.includes('anime') || lower.includes('manga') || lower.includes('cartoon') || lower.includes('chibi') || lower.includes('illustrated'))
        return 'anime';
    if (lower.includes('cyberpunk') || lower.includes('futuristic') || lower.includes('neon') || lower.includes('sci-fi') || lower.includes('blade runner') || lower.includes('dystopian'))
        return 'cyberpunk';
    if (lower.includes('city') || lower.includes('skyline') || lower.includes('building') || lower.includes('architecture') || lower.includes('lagos') || lower.includes('abuja') || lower.includes('nairobi') || lower.includes('accra'))
        return 'architecture';
    if (lower.includes('african') || lower.includes('nigeria') || lower.includes('ghana') || lower.includes('tribal') || lower.includes('traditional') || lower.includes('yoruba') || lower.includes('igbo') || lower.includes('hausa') || lower.includes('zulu') || lower.includes('benin'))
        return 'african_art';
    if (lower.includes('lion') || lower.includes('elephant') || lower.includes('tiger') || lower.includes('leopard') || lower.includes('cheetah') || lower.includes('gorilla') || lower.includes('eagle') || lower.includes('wildlife') || lower.includes('animal') || lower.includes('wolf'))
        return 'wildlife';
    if (lower.includes('portrait') || lower.includes('person') || lower.includes('woman') || lower.includes('man') || lower.includes('face') || lower.includes('headshot'))
        return 'portrait';
    if (lower.includes('cinematic') || lower.includes('movie') || lower.includes('film') || lower.includes('dramatic') || lower.includes('epic') || lower.includes('fantasy') || lower.includes('warrior'))
        return 'cinematic';
    if (lower.includes('abstract') || lower.includes('pattern') || lower.includes('texture') || lower.includes('geometric') || lower.includes('fractal'))
        return 'abstract';
...
function buildEnhancedPrompt(rawPrompt, mode, brandName) {
    ...
    switch (mode) {
        case 'wildlife': {
            ...
            if (lower.includes('sit') || lower.includes('perch'))
                actionConstraint = "perched still on a branch, stationary pose";
            if (lower.includes('run'))
                actionConstraint = "sprinting with athletic stride, high-speed motion blur, paws hitting ground";
...
function buildVisualRequest(rawPrompt) {
    ...
    const negativePrompt = exports.NEGATIVE_PROMPTS[mode];
    const models = exports.MODE_MODELS[mode];
...
// ── Quality scorer ─────────────────────────────────────────────────────────
function scoreImageQuality(bytes, mode) {
    const minExpected = {
...
//# sourceMappingURL=visualIntelligence.js.map