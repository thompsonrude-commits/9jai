...
exports.generateLogoSVG = generateLogoSVG;
exports.svgToBase64DataUrl = svgToBase64DataUrl;
function detectColorScheme(prompt) {
...
function detectLogoStyle(prompt) {
    const lower = prompt.toLowerCase();
    if (lower.includes('minimal') || lower.includes('clean') || lower.includes('simple'))
        return 'modern_minimal';
    if (lower.includes('tech') || lower.includes('ai') || lower.includes('digital') || lower.includes('cyber') || lower.includes('hex'))
        return 'tech_hex';
    if (lower.includes('shield') || lower.includes('security') || lower.includes('protect') || lower.includes('trust'))
        return 'shield';
    return 'circular_badge';
}
// ── SVG generators ─────────────────────────────────────────────────────────
function generateCircularBadgeSVG(brandName, initials, colors) {
    const size = 800;
    const cx = 400;
    const cy = 400;
    const outerR = 340;
...
function generateModernMinimalSVG(brandName, initials, colors) {
    const size = 800;
    const cx = 400;
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
  <defs>
...
function generateTechHexSVG(brandName, initials, colors) {
    const size = 800;
    const cx = 400;
    const cy = 360;
    const hexR = 200;
 ...
 */
function generateBrandKit(brandName, prompt) {
...
function generateLogoSVG(brandName, prompt) {
    const colors = detectColorScheme(prompt);
    const style = detectLogoStyle(prompt);
    const initials = getInitials(brandName);
...
//# sourceMappingURL=logoEngine.js.map