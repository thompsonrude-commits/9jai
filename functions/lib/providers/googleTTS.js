...
exports.aiSpeak = exports.GOOGLE_TTS_KEY = void 0;
exports.synthesizeNigerianSpeech = synthesizeNigerianSpeech;
 ...
 */
exports.GOOGLE_TTS_KEY = (0, params_1.defineSecret)('GOOGLE_TTS_KEY');
 ...
 */
exports.aiSpeak = (0, https_1.onCall)({
    region: "us-central1",
    cors: true,
    maxInstances: 10,
    secrets: [exports.GOOGLE_TTS_KEY],
}, async (request) => {
    const { text, assistantId = 'nosa' } = request.data;
    if (!text) {
        throw new https_1.HttpsError('invalid-argument', 'The function must be called with a "text" argument.');
    }
    try {
        ...
        throw new https_1.HttpsError('internal', 'Failed to synthesize speech via Google Cloud TTS');
    }
});
async function synthesizeNigerianSpeech(text, assistantId) {
    const key = exports.GOOGLE_TTS_KEY.value();
    ...
    try {
        ...
        if (!res.ok) {
            ...
            if (res.status === 429 || err.includes('quota')) {
                ...
                const retryRes = await fetch(`https://texttospeech.googleapis.com/v1/text:synthesize?key=${key}`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
...
//# sourceMappingURL=googleTTS.js.map